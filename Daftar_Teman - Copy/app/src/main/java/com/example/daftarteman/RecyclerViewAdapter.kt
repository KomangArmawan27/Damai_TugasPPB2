...
class RecyclerViewAdapter(private val listdata_teman: ArrayList<data_teman>, context: Context) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val Role: String? = listdata_teman.get(position).role
        val Tim: String? = listdata_teman.get(position).tim

        holder.Nama.text = "Nama: $Nama"
        holder.Alamat.text = "Alamat: $Alamat"
        holder.NoHP.text = "NoHP: $NoHP"
        holder.Role.text = "Role: $Role"
        holder.Tim.text = "Tim: $Tim"
                    ...
                    alert.setItems(action, DialogInterface.OnClickListener { dialog, i ->
                        when (i) {
                            0 -> {
                                ...
                                bundle.putString("dataRole", listdata_teman[position].role)
                                bundle.putString("dataTim", listdata_teman[position].tim)
